		...
		_, err = jsonparser.ArrayEach(c,
			func(value []byte, dt jsonparser.ValueType, offset int, err error) {
				...
				if renameTo != "" {
					...
					*s.content, errCb = jsonparser.Set(*s.content, []byte(toSet), append(paths, idxStr, "INDEX_DEC")...)
					if errCb != nil {
						return
					}
				}
...
func (s *ClaRiskIndexes) summary() error {
	...
	*s.content, err = jsonparser.Set(*s.content, summaryBytes, "impExpEntReport", "riskIndexesSummary")
	if err != nil {
		return err
	}
	return nil
