...
// ClaSellingDetailSummary 4.4.1/4.4.2 交易明细总结
type ClaSellingDetailSummary struct {
	content   *[]byte
	contentId int64
}

func (s *ClaSellingDetailSummary) SetContent(content *[]byte, contentId int64) {
	s.content = content
	s.contentId = contentId
}

func (s *ClaSellingDetailSummary) Collating() error {
	...
	content, err := jsonparser.Set(*s.content, summaryBytes, "impExpEntReport", "sellingDetailSummary")
	if err != nil {
...
func (s *ClaSellingDetailSummary) subSummary1() (string, error) {
	sql :=
	...
	return fmt.Sprintf("近12个月前五大客户合计占比%.2f%%，前20下游客户合计占比%.2f%%，客户集中度%s。",
		top5SumRatio, top20SumRatio, desc), nil
}

func (s *ClaSellingDetailSummary) subSummary2() (string, error) {
	...
	if err := db.Model(&modelRi).
		Select("index_value").
		Where("content_id = ? and index_dec = ?", s.contentId, "客户稳定性").
		First(&stabRatioStr).
		Error; err != nil {
		return "", err
	}
	...
	return fmt.Sprintf(
		"近12个月与前12个月重要客户金额占比重合程度比值为%.2f%%,客户稳定性%s", stabRatio, desc), err
}

func (s *ClaSellingDetailSummary) subSummary3(idx int) (string, error) {

	modelTd := models.RcTradesDetail{}
	db := sdk.Runtime.GetDbByKey(modelTd.TableName())

	var m12TopCus, m12TopCusAtM24 models.RcTradesDetail
	if err := db.Model(&modelTd).
		Where("content_id = ? and detail_type = ?", s.contentId, 1).
	...
	if err := db.Model(&modelTd).
		Where("content_id = ? and detail_type = ?", s.contentId, 2).
		Where("enterprise_name = ?", m12TopCus.EnterpriseName).
		First(&m12TopCusAtM24).
		Error; err != nil {
		return "", err
	}
	...
	if err := db.Table(modelTd.TableName()).Raw(
		`select ranking
			...
			where t.id = ?;`, s.contentId, m12TopCusAtM24.Id).
		First(&ranking).Error; err != nil {
		return "", err
	}

	fM12Ratio, err := strconv.ParseFloat(strings.Replace(m12TopCus.RatioAmountTax, "%", "", -1), 64)
	if err != nil {
		return "", err
	}
	fM24Ratio, err := strconv.ParseFloat(strings.Replace(m12TopCusAtM24.RatioAmountTax, "%", "", -1), 64)
	if err != nil {
		return "", err
	}
	...
	if annualChangedRatio > 0 {
		if 10 < annualChangedRatio && annualChangedRatio <= 30 {
			annualChangedDesc = "近期订单<font color= #EF5644>小幅度提高</font>"
		} else if 30 < annualChangedRatio && annualChangedRatio <= 100 {
			annualChangedDesc = "近期订单<font color= #EF5644>大幅度提高</font>"
		} else if 100 < annualChangedRatio && annualChangedRatio <= 170 {
			annualChangedDesc = "近期订单<font color= #EF5644>约提高了1倍</font>"
		} else if 170 < annualChangedRatio {
			x := int((annualChangedRatio-100)/70) + 1
			annualChangedDesc = fmt.Sprintf("近期订单约提高了%d倍", x)
		}
	} else if annualChangedRatio < 0 {
		if -10 > annualChangedRatio && annualChangedRatio >= -40 {
			annualChangedDesc = "近期订单<font color= #67C23A>小幅度下降</font>"
		} else if -40 > annualChangedRatio && annualChangedRatio >= -60 {
			annualChangedDesc = "近期订单<font color= #67C23A>减少约一半</font>"
		} else if -60 > annualChangedRatio && annualChangedRatio >= -90 {
			annualChangedDesc = "近期订单<font color= #67C23A>约大幅减少</font>"
		} else if -90 > annualChangedRatio && annualChangedRatio >= -100 {
			annualChangedDesc = "近期基本<font color= #67C23A>没有订单</font>"
		}
	}
	if annualChangedDesc == "" {
		annualChangedDesc = "近期订单<font color= #E6A23C>基本稳定</font>"
	}

	return fmt.Sprintf("近12个月第%d大客户为'%s'，销售占比为%s，近24个月此客户销售占比为%s，销售排名第%d，近12个月内销售占比相较于24个月内变化%.2f%%。%s。",
		idx, m12TopCus.EnterpriseName, m12TopCus.RatioAmountTax, m12TopCusAtM24.RatioAmountTax, ranking, annualChangedRatio, annualChangedDesc), nil
}

func (s *ClaSellingDetailSummary) subSummary4() (string, error) {
	...
	return fmt.Sprintf("近12个月前20下游客户中国内企业客户有%d家，成立时间在2000年之前%d家，占比%.2f%%，2000-2009年有%d家，占比%.2f%%，2010-2019年有%d家，占比%.2f%%，2020年-%d年有%d家，占比%.2f%%，%s。",
		total, c1, c1p, c2, c2p, c3, c3p, maxTime.Year(), c4, c4p, desc), nil

}

func (s *ClaSellingDetailSummary) countByEstablishDate(minTime time.Time, maxTime time.Time) (int64, error) {
	modelTd := models.RcTradesDetail{}
	db := sdk.Runtime.GetDbByKey(modelTd.TableName())
	var count int64
		...
		`select count(distinct enterprise_info.usc_id) as n
			from (select usc_id, enterprise_wait_list.enterprise_name as enterprise_name
				  ...
				  where rc_trades_detail.content_id = ?
					and rc_trades_detail.detail_type = 1
...
func (s *ClaSellingDetailSummary) getContent() (*models.RcOriginContent, error) {
