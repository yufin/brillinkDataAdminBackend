...
func collateDependencyForContent(contentId int64) error {
	...
	err = dbContent.Model(models.RcOriginContent{}).First(&tbContent, contentId).Scan(&tbContent).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return nil
		}
...
func collateContent(contentId int64, nameDataMap map[string]*dto.SubjoinData, evalTime time.Time) error {
	...
	insertReq := dto.RcProcessedContentInsertReq{
		ContentId:  contentId,
		Content:    string(csb),
		StatusCode: 1,
	}
	var pc models.RcProcessedContent
	insertReq.Generate(&pc)
	if err := dbProc.Model(&models.RcProcessedContent{}).Create(&pc).Error; err != nil {
		return err
	}
	return nil
}
