...
// 需认证的路由代码
func registerSysRoleRouter(v1 *gin.RouterGroup, authMiddleware *jwtauth.GinJWTMiddleware) {
	api := apis.SysRole{}
	...
	r2 := v1.Group("").Use(authMiddleware.MiddlewareFunc())
	{
		r2.GET("/role-option", api.GetOption)
	}
