...
type SysUserGetPageReq struct {
	...
	StartTime      string `form:"startTime" search:"type:gte;column:created_at;table:sys_user" comment:"创建时间"`
...
type ResetSysUserPwdReq struct {
	UserId   int    `json:"userId" comment:"用户ID" vb:"$>0"` // 用户ID
	Password string `json:"password" comment:"密码" vb:"len($)>0"`
	common.ControlBy
}
...
type UpdateSysUserPwdReq struct {
	UserId      int64  `json:"-" comment:"用户ID"` // 用户ID
	Password    string `json:"newPassword" comment:"密码" vb:"len($)>0"`
	OldPassword string `json:"oldPassword" gorm:"-" vd:"len($)>0"`
	common.ControlBy
}

func (s *UpdateSysUserPwdReq) GetId() interface{} {
	return s.UserId
}
...
func (s *SysUserGetResp) Generate(model *models.SysUser) {
	s.UserId = model.UserId
	s.Username = model.Username
	s.Password = model.Password
	s.NickName = model.NickName
	s.Phone = model.Phone
	s.RoleId = model.RoleId
	s.Avatar = model.Avatar
	s.Sex = model.Sex
	s.Email = model.Email
	s.DeptId = model.DeptId
	s.PostId = model.PostId
