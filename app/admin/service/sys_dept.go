...
// Get 获取SysDept对象
func (e *SysDept) Get(r *dto.SysDeptGetReq) (model *models.SysDept, err error) {
	model, err = service.Get[models.SysDept](e, r)
	if err != nil {
...
// Insert 创建SysDept对象
func (e *SysDept) Insert(c *gin.Context, r *dto.SysDeptInsertReq) error {
	var err error
...
// Update 修改SysDept对象
func (e *SysDept) Update(c *gin.Context, r *dto.SysDeptUpdateReq) error {
	var err error
...
// Remove 删除SysDept
func (e *SysDept) Remove(c *gin.Context, r *dto.SysDeptDeleteReq) error {
	var err error
	var data models.SysDept

	db := e.Orm.Model(&data).Delete(&data, r.GetId())
	if db.Error != nil {
		err = db.Error
...
func (e *SysDept) SetTree(c *dto.SysDeptGetPageReq) (m []dto.DeptTree, err error) {
	var list []models.SysDept
	err = e.getList(c, &list)

	m = make([]dto.DeptTree, 0)
	for i := 0; i < len(list); i++ {
		if list[i].ParentId != 0 {
			continue
		}
...
// Call 递归构造组织数据
func treeCall(deptList *[]models.SysDept, dept dto.DeptTree) dto.DeptTree {
	list := *deptList
	min := make([]dto.DeptTree, 0)
	for j := 0; j < len(list); j++ {
		if dept.Id != list[j].ParentId {
			continue
		}
		mi := dto.DeptTree{Id: list[j].DeptId, Label: list[j].DeptName, Children: []dto.DeptTree{}}
		ms := treeCall(deptList, mi)
		min = append(min, ms)
	}
	dept.Children = min
	return dept
