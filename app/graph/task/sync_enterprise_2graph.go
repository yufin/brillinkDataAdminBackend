...
func (t SyncGraphTask) Exec(arg interface{}) error {
	if !config.ExtConfig.Graph.Neo4j.Activate {
		log.Info("Neo4j Drive is not activated")
		return nil
	}
...
// AssigningTask Pub uscId where not exist in graphDb
...
// SyncGraph Sync Graph By Given uscId from Table enterprise...
