...
func (e *ExecJob) Run() {
	...
	//ws.SendAll(str)
	log.Info("JobCore %s exec success , spend :%v", e.Name, latencyTime)
	return
}
...
LOOP:
	if count < retryCount {
		...
		if err != nil {
			// 如果失败暂停一段时间重试
			log.Error("mission failed! ", err)
			log.Info("Retry after the task fails %d seconds! %s \n", (count+1)*5, str)
...
func setup(key string, db *gorm.DB) {
	...
	if err != nil {
		log.Error(" JobCore init error", err)
	}
	if len(jobList) == 0 {
	...
	if err != nil {
		log.Error(" JobCore remove entry_id error", err)
	}
