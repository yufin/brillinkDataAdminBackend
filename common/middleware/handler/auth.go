...
import (
	...
	configx "github.com/go-admin-team/go-admin-core/sdk/config"
...
func Authenticator(c *gin.Context) (interface{}, error) {
	...
	if err != nil {
		...
		panic(exception.New(exception.GetOrmFail, err))
	...
	if err = c.ShouldBind(&loginVals); err != nil {
		...
		panic(exception.New(exception.LoginGetUserNameFail, err))
		return nil, jwt.ErrMissingLoginValues
	}
	if (configx.ApplicationConfig.Mode != "dev" && configx.ApplicationConfig.Mode != "demo") || loginVals.Type != "account" {
		if !captcha.Verify(loginVals.UUID, loginVals.Code, true) {
			err = errors.WithStack(err)
			username = loginVals.Username
			panic(exception.New(exception.LoginInvalidVerificationCodeFail, err))
			return nil, jwt.ErrInvalidVerificationode
		}
	}
	userInfo, role, err := loginVals.GetUser(db)
	if err == nil {
		username = loginVals.Username
		return map[string]interface{}{"user": userInfo, "role": role}, nil
	} else {
		if userInfo.UserId > 0 {
			err = errors.WithStack(err)
			panic(exception.New(exception.LoginUsernameOrPasswordFail, err))
		}
		err = errors.WithStack(err)
		panic(exception.New(exception.LoginFail, err))
	}
	return nil, jwt.ErrFailedAuthentication
}
...
func LoginLogToDB(c *gin.Context, status string, msg string, username string) {
	if !configx.LoggerConfig.EnabledDB {
		return
	}
	log := apis.GetRequestLogger(c)
	...
	message, err := sdk.Runtime.GetStreamMessage("", string(global.LoginLog), l)
	if err != nil {
