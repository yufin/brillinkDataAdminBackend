...
// CasbinExcludeInit 默认添加对应的排除路由
func CasbinExcludeInit(mp map[string]*gorm.DB) {
	list := make([]models.SysApi, 0)
	for _, db := range mp {
		err := db.Model(&models.SysApi{}).Where("type=?", "SYS").Find(&list).Error
		if err != nil {
			log.Error("CasbinExcludeInit", err)
			break
		}
		for _, v := range list {
			CasbinExclude = append(CasbinExclude, UrlInfo{Url: v.Path, Method: v.Method})
		}
	}
}
